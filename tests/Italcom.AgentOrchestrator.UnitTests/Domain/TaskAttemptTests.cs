...
namespace Italcom.AgentOrchestrator.UnitTests.Domain
{
    public sealed class TaskAttemptTests
    {
        ...
        [Fact]
        public void Constructor_throws_on_null_modelId() => Assert.Throws<ArgumentNullException>(() => new TaskAttempt(
        ...
        public void Cancel_from_Completed_fails()
        {
            var attempt = CreateQueued();
            _ = attempt.Start();
            _ = attempt.Complete(new Usage(0, 0), Cost.Zero(), true, true, true);
            var result = attempt.Cancel();
            Assert.True(result.IsFailure);
        }
        ...
        public void IsSuccessful_true_when_completed_and_all_flags_true()
        {
            var attempt = CreateQueued();
            _ = attempt.Start();
            _ = attempt.Complete(new Usage(100, 200), new Cost(0.05m, "USD"), true, true, true);
        ...
        public void IsSuccessful_false_when_build_fails()
        {
            var attempt = CreateQueued();
            _ = attempt.Start();
            _ = attempt.Complete(new Usage(0, 0), Cost.Zero(), buildSuccess: false, testSuccess: true, reviewApproved: true);
            Assert.False(attempt.IsSuccessful);
        }
        ...
        public void IsSuccessful_false_when_test_fails()
        {
            var attempt = CreateQueued();
            _ = attempt.Start();
            _ = attempt.Complete(new Usage(0, 0), Cost.Zero(), true, false, true);
            Assert.False(attempt.IsSuccessful);
        }
        ...
        public void IsSuccessful_false_when_review_not_approved()
        {
            var attempt = CreateQueued();
            _ = attempt.Start();
            _ = attempt.Complete(new Usage(0, 0), Cost.Zero(), true, true, false);
            Assert.False(attempt.IsSuccessful);
        }
    }
}
